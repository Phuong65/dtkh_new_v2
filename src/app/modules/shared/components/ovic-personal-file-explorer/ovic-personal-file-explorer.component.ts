...
export class OvicPersonalFileExplorerComponent implements OnInit , OnDestroy {
	...
	ngOnInit() : void {
		...
		this.saveButton   = this.buttonLabels?.save ? this.buttonLabels.save : '<i class="fa fa-floppy-o" ></i>Lưu và đóng';
		this.cancelButton = this.buttonLabels?.cancel ? this.buttonLabels.cancel : '<i class="fa fa-times" ></i>Thoát';
		void this.init();
	}
		...
		if ( validateFiles.length > this.maxUploadingFiles ) {
			...
			validateFiles.forEach( ( file , index ) => {
				if ( this.server === 'serverFile' ) {
					...
					setTimeout( () => {
						this.fileService.gdUploadFile( file.file , this.rootFolder ).subscribe( {
							next  : () => {
								file.uploaded = true;
								file.message  = 'Upload thành công';
								setTimeout( () => this.removeFileFromUploadingList( null , file.id ) , 3000 + ( index * 100 ) );
	...
	createNewStore() {
		...
		this.fileService.gdCreateFolder( { name , parents } ).pipe(
			mergeMap( folderInfo => {
