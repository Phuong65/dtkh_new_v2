...
@Component({standalone: false, 
    selector: 'ovic-groups-checkbox-v2',
    templateUrl: './ovic-groups-checkbox-v2.component.html',
    styleUrls: ['./ovic-groups-checkbox-v2.component.css']
})
export class OvicGroupsCheckboxV2Component implements OnInit, OnChanges, OnDestroy, AfterViewInit {
    ...
    ngOnInit(): void {
        if ((this.defaultValue) && this.options && this.optionId) {
            this.default = this.defaultValue;
        }
    }
    ...
    onChooseItem(item) {
        if (!item['disabled']) {
            if (this.unSelect) {
                const index = this.default.findIndex(m => m.toString() === item.toString())
                if (index !== -1) {
