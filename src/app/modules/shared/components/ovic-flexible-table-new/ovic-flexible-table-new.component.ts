...
export class OvicFlexibleTableNewComponent implements OnInit {
    ...
    @Input() headSearch: OvicFlexibleTableNewHeadSearch = {
        ...
        button: '<i class="fa fa-search" ></i>Tìm kiếm'
    };
    ...
    @Output() onFieldClick = new EventEmitter<any[]>();
    ...
    btnToggleAllRowChecked() {
        if (this.data && this.data.length > 0) {
            if (this.rowsChecked.length === this.data.length) {
                this.data.forEach(f => {
                    f['__oft_row_checked'] = false;
                })
                this.rowsChecked = [];
            } else {
                this.data.forEach(f => {
                    f['__oft_row_checked'] = true;
                })
    ...
    onClickAble(column, loop) {
        if (column.fieldClick) {
            this.onFieldClick.emit(loop);
        }
    }
}
