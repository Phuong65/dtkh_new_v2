...
export class TreeCustomComponent implements OnInit, OnChanges {
    ...
    @Input() merNodeAndIcon = false;

    @Output() selectNode = new EventEmitter<TREE>();

    @Output() closeAndOpenNode = new EventEmitter<TREE>();

    @Output() onEditNode = new EventEmitter<TREE>();

    @Output() onDeleteNode = new EventEmitter<TREE>();
    ...
    editNodeAction(node: TREE) {
        this.onEditNode.emit(node);
    }

    deleteNodeAction(node: TREE) {
