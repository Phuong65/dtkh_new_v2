...
export class FileListLocalOnTableComponent implements OnInit , OnChanges {
	...
	async onChangesFileChooser( event : Event ) {
		if ( event.target[ 'files' ].length ) {
			try {
				const uploaded = await this.uploadFiles( event.target[ 'files' ] , this.auth.userDonViId , this.auth.user.id );
				if ( uploaded.error ) {
	...
	async uploadFiles( fileList : FileList , donViId : number , userId : number ) : Promise<{ error : boolean, data : SimpleFileLocal[] }> {
		return new Promise<{ error : boolean, data : SimpleFileLocal[] }>( resolve => {
			...
			for ( let i = 0 ; i < fileList.length ; i++ ) {
				...
				setTimeout( () => this.fileService.uploadFile( file , donViId , userId ).subscribe( {
					next  : ( f ) => {
						result.data.push( {
							id    : f.id ,
							name  : f.name ,
