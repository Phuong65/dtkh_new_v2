...
export class OvicFileExplorerComponent implements OnInit {
	 ...
	 * *****************************************************/
	private getData( serverName : string , search : SearchInfo ) : Observable<OvicFile[] | OvicDriveFile[]> {
			...
			validateFiles.forEach( ( file , index ) => {
				if ( this.server === 'serverFile' ) {
					...
					setTimeout( () => {
						...
						this.fileService.gdUploadFile( file.file , this.auth.cloudStore ).subscribe(
							{
								next  : () => {
									file.uploaded = true;
									file.message  = 'Upload thành công';
