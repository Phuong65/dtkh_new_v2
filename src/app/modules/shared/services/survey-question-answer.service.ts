...
export class SurveyQuestionAnswerService {
    ...
    private api = getRoute('survey-question-answers/');
    ...
    countAnswer(id: string): Observable<number> {
        const fromObject: IctuQueryParams = { limit: 1, paged: 1, include: id, include_by: 'survey_plan_id' };
        const params: HttpParams = this.httpHelper.paramsConditionBuilder([], new HttpParams({ fromObject }));
        return this.http.get<Dto>(this.api, { params }).pipe(map(res => res.recordsFiltered));
    }

    countAnswerSurvey(id: string): Observable<number> {
        const fromObject: IctuQueryParams = { limit: 1, paged: 1, include: id, include_by: 'survey_id' };
        const params: HttpParams = this.httpHelper.paramsConditionBuilder([], new HttpParams({ fromObject }));
        return this.http.get<Dto>(this.api, { params }).pipe(map(res => res.recordsFiltered));
    }
