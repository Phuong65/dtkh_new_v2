...
import { SurveyPlan } from "../models/survey-plan";
@Injectable({
    providedIn: 'root'
})
export class SurveyPlanService {

    private api = getRoute('survey-plans/');
    ...
    postData(info: Partial<SurveyPlan>): Observable<Dto> {
        return this.http.post<Dto>(this.api, info);
    }

    putData(info: Partial<SurveyPlan>, id: string): Observable<Dto> {
