...
export class ThibackupShiftStudentsService {

    api = getRoute( 'thibackup-shift-students/' );
    ...
    lockLogin ( shift_id: number, room: string, shift_studnet_id: number = null ) {
        let params = new HttpParams().set( 'room', room );

        if ( shift_studnet_id ) {
            params = params.set( 'id', shift_studnet_id.toString() );
        }
        return this.http.post<Dto>( this.api.concat( 'khoa-dang-nhap/', shift_id.toString() ), {}, { params: params } ).pipe(
            map( res => res )
        );
    }
