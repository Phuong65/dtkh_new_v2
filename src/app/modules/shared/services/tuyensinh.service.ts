...
import { Tuyensinh } from '../../shared/models/tuyensinh';
...
export class TuyensinhService {

    private api = getRoute('registrations/');
    ...
    addTuyensinh(data: any): Observable<any> {
        return this.http.post<Dto>(this.api, data).pipe(
            map(res => res.data)
        );
    }

    updateTuyensinh(deXuatId: any, data: any): Observable<any> {
        return this.http.put<Dto>(this.api.concat(deXuatId.toString()), data).pipe(
            map(res => res.data)
        );
    }

    getAllTuyensinhByUserCreate(user_id: number): Observable<Tuyensinh[]> {
        return this.http.get<Dto>(this.api.concat('?condition=creator_id,=,', user_id.toString())).pipe(
            map(res => res.data)
        );
    }

    getAllTuyensinh(): Observable<Tuyensinh[]> {
    ...
    deleteTuyensinh(id: number): Observable<any> {
        return this.http.delete<Dto>(this.api.concat(id.toString())).pipe(
            map(res => res.data)
        );
    }

    getTuyensinhByCol(col: string, item: string): Observable<Tuyensinh[]> {
    ...
    getTuyensinhByItem(item: string, col: string): Observable<Tuyensinh[]> {
    ...
    getTuyensinhLenght(): Observable<any> {
    ...
    getTuyensinhPage(start: number, limit: number): Observable<Tuyensinh[]> {
    ...
    getTuyensinhByCols(condition: HttpParams): Observable<Tuyensinh[]> {
    ...
    getTuyensinhByPageNew(option: ConditionOption): Observable<{ data: Tuyensinh[], recordsFiltered: number }> {
