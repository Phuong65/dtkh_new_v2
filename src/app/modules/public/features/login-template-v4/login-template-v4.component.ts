...
import { APP_CONFIGS, google_client_id } from '@env';
        ...
        if (this.auth && this.auth.roles && this.auth.roles.length === 1 && this.auth.roles[0].name === 'student') {
            ...
            this.fileService.getFileLocalAsBlob('..\\assets\\json\\front_end.json').subscribe(_res => {
                ...
                reader.onloadend = (event) => {
                    ...
                    const front_end = json.url;
                    this.isLoading = false;
                    this.currentLoginButton.isLoading = false;
                    this.notificationService.confirm('<div><div>Đây là trang dành cho giảng viên, bạn không thể truy cập vào trang này</div><div>Chuyển đến trang dành cho sinh viên <a href="' + front_end + '">tại đây</a></div></div>', 'Thông báo').then(() => null, () => null)
                };
            })
            return;
        }
