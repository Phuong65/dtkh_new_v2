...
export class InputboxEditorComponent implements OnInit, OnChanges {
    ...
    /** Input trống cuối danh sách dùng để thêm đáp án mới, key = q.id */
    newAnswerInput: { [qId: string]: string } = {};

    /** Input sửa đáp án đã có, key = `${q.id}|${index}` */
