...
export interface student_import_shift {
...
export class SinhvienCathiHvuComponent implements OnInit {
    ...
    list_room: ThiShiftStudents[];

    selectedRoom: ThiShiftStudents;

    searRoom: string;

    list_student_in_room: ThiShiftStudents[];
                    ...
                    if (Object.keys(wb.Sheets) && Object.keys(wb.Sheets).length) {
                        ...
                        Object.keys(wb.Sheets).forEach(f => {
                            ...
                            data.forEach(f => {
                                ...
                                if (check_true_row && !isNaN(f[0])) {
                                    if (!object_check[f[2].toString().trim().toLowerCase()]) {
                                        object_check[f[2].toString().trim().toLowerCase()] = true;
                                        const student: student_import_shift = {
                                            stt: Number(f[0].toString().trim()),
                                            sbd: f[1].toString().trim(),
                                            masv: f[2].toString().trim().toLowerCase(),
                                            full_name: f[3].toString().trim().concat(' ', f[4].toString().trim()),
                                            name_string: f[4].toString().trim(),
                                            birthday: null,
                                            room: f[5] ? f[5].toString().trim() : null,
                ...
                next: (_user_res) => {
                    ...
                    data_import.forEach(f => {
                        ...
                        if (index !== -1) {
                            ...
                            f.birthday = _user_res[index].birthday;
                        } else {
                            f['status'] = -1
                        }
                    })
            ...
            this.notificationService.confirm("Thầy/Cô có chắc chắn bắt đầu quá trình import không?", "Xác nhận hành động", [BUTTON_YES, BUTTON_NO]).then(a => {
                if (a.name === 'yes') {
                    ...
                    data_can_import.forEach(f => {
                        const data_student: ThiShiftStudents = {
                            shift_id: f.shift_id,
                        ...
                        if (request[i].length < 6) {
                            request[i].push(this.thiShiftStudentsService.addThiShiftStudents(data_student).pipe(
                                map(a => {
                                    f['status'] = 1;
                                    return;
                                }),
    ...
    onSelectedRoom(room: ThiShiftStudents) {
    ...
    async onLoadStudentByRoom(page: number) {
        const condition_student: ConditionOption = {
            ...
            set: [
                ...
                { label: 'select', value: 'id,student_id,thi_question_bank_tn_id,id,ordering,sbd,room' },
        ...
        this.thiShiftStudentsService.getThiShiftStudentsByPageNew(condition_student).pipe(mergeMap(a => {
            return of(a);
        })).subscribe({
            next: (_student) => {
    ...
    deleteOneStudent(student: ThiShiftStudents) {
        this.notificationService.confirmDelete().then(a => {
            if (a) {
                this.notificationService.isProcessing(true);
                this.thiShiftStudentsService.deleteThiShiftStudents(student.id).subscribe({
                    next: () => {
                        this.loadStudentInShift();
    ...
    deleteAllStudentInRoom() {
        this.notificationService.confirmDelete().then(a => {
            if (a) {
                this.notificationService.isProcessing(true);
                if (this.selectedRoom.id) {
                    this.thiShiftStudentsService.deleteThiShiftStudentsByCol(this.shiftSelected.id.toString(), 'shift_id').subscribe({
                        next: () => {
                            this.notificationService.toastSuccess("Xóa thành công");
                            this.loadStudentInShift();
                        },
                    ...
                    this.thiShiftStudentsService.getThiShiftStudentsByPageNew(condition_student).pipe(mergeMap(a => {
                        ...
                        if (thi_student_ids.length) {
                            return this.thiShiftStudentsService.deleteThiShiftStudents([... new Set(thi_student_ids)].toString()).pipe(mergeMap(s => {
                                return of(null);
                            }))
                        }
    ...
    addOneStudent() {
        if (this.student_one_add) {
            if (this.student_one_add['SBD']) {
                ...
                this.thiShiftStudentsService.addThiShiftStudents(data).subscribe({
                    next: () => {
                        this.loadStudentInShift();
    ...
    downLoadFileSinhVienEx() {
        this.fileService.getFileContent('..\\assets\\files\\mau_file_import_hvu\\mau_import_sinhvien_thi_kthp.xlsx').subscribe(res => {
            saveAs(res, 'Mẫu import danh sách sinh viên thi kết thúc học phần.xlsx');
        });
    }
}
