import { CourseQuestionFormService } from './../../../../shared/services/course-question-form.service';
...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, FormsModule, SharedModule, TableModule, PaginatorModule, ReactiveFormsModule, CalendarModule, MatMenuModule, OverlayPanelModule, DropdownModule, DialogModule, MatProgressBarModule],
    templateUrl: './form-de-chitiet.component.html',
    styleUrls: ['./form-de-chitiet.component.css']
})
export class FormDeChitietComponent implements OnInit, OnChanges {
    ...
    initFormDetail() {
        this.selectedForm['question_take_total'] = null;
        this.list_course_plan = null;
        switch (this.selectedForm.av) {
            case 0:
                this.loadType0();
                break;
            case 1:
    ...
    loopAddForm(request: Observable<any>[]): Observable<any> {
        return from(request).pipe(
            concatMap((obs, idx) => {
                this.progressValue = (idx + 1) / request.length * 100;
                return obs;
            }),
            last(),
        );
    ...
    loadType0() {
        const condition_plan: ConditionOption = {
            condition: [
                { conditionName: 'course_id', condition: OvicQueryCondition.equal, value: this.selectedForm.course_id.toString(), orWhere: 'and' },
        ...
        const condition_form_detail: ConditionOption = {
            condition: [
                { conditionName: 'form_id', condition: OvicQueryCondition.equal, value: this.selectedForm.id.toString(), orWhere: 'and' },
            ],
            set: [
                { label: 'limit', value: '-1' },
            ],
        ...
        forkJoin([
            ...
            this.ThiFormDetailsService.getThiFormDetailsByPageNew(condition_form_detail),
    ...
    setNumberQuestiontype0(item: PlanActivityCdr, cdr: any, math_total = true) {
        ...
        if (math_total) {
            ...
            this.selectedForm['question_take_total'] = s_total ? s_total : 0;
            this.selectedForm['question_take_total_private'] = s_total_private ? s_total_private : 0;
        }
    }
    ...
    saveType0() {
        ...
        request.push(this.ThiFormDetailsService.deleteThiFormDetailsByCol(this.selectedForm.id.toString(), 'form_id'))
        ...
        this.list_course_plan.forEach(f => {
            ...
            Object.keys(f['question_take']).forEach(o => {
                if (f['question_take'][o]) {
                    const data = {
                        course_id: this.selectedForm.course_id,
                        form_id: this.selectedForm.id,
                        week: f.week,
                        cdr: o,
                        total_question_take: f['question_take'][o]
                    }
                    request.push(this.ThiFormDetailsService.addThiFormDetails(data))
                }
            })
        })
        ...
        if (check) {
            ...
            if (request.length > 1) {
                ...
                this.loopAddForm(request).subscribe({
                    next: () => {
    ...
    setNumberQuestiontype2() {
        ...
        this.selectedForm['question_take_total'] = s;

        this.selectedForm['question_take_total_private'] = s_private;
    }
