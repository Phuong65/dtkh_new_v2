...
export class XuatBaithiComponent implements OnInit {
    ...
    constructor(
        private thiShiftsService: ThibackupShiftsService,
        ...
        private thiShiftRoomssService: ThibackupShiftRoomssService,
        private roleService: RoleService,
        private userService: UserService,
        private thiShiftStudentsService: ThibackupShiftStudentsService,
        private thiFormService: ThibackupFormService,
        private thiLogsService: ThibackupLogsService,
        private fileService: FileService,
        private exportPointForIUService: ExportPointForIUService,
        private courseQuestionsService: CourseQuestionsService,
        private thiStudentAnswerService: ThibackupStudentAnswerService,
        private latexToImageSvgService: LatexToImageSvgService
    ) {
    ...
    loadShiftRoom(download_shift: boolean = false) {
        ...
        const condition_log: ConditionOption = {
            condition: [
                { conditionName: 'thibackup_shift_id', condition: OvicQueryCondition.equal, value: this.selectedShift.id.toString() }
            ],
            set: [
                { label: 'limit', value: '-1' },
                { label: 'order', value: 'ASC' },
    ...
    loadListShiftStudent(download: boolean = false, download_shift: boolean = false) {
        const condition_shift_student: ConditionOption = {
            ...
            set: [
                ...
                { label: 'select', value: 'questions,tracking,id,shift_id,student_id,sbd,ordering,room,status,completed,locked,point,time_remaining,thibackup_question_bank_tn_id,student_user_id,violation_of_exam,progress,total,warning,submited_by' },
            ],
                    ...
                    this.loopGetDataStudentAnswer(request_answer, 0, []).subscribe({
                        next: async (_student_answer) => {
                            ...
                            shift_student.forEach(async f => {
                                ...
                                f['_test'] = await this.latexToImageSvgService.convertHtmlForWord(f['_test']);
    ...
    async convertToPDF(shift_student: ThiShiftStudents[], stringStyleCss: string, key: number, zip, shiftFolder, text) {
        ...
        // latexToImageSvgService
        // const content_test = shift_student[key]['_test'].replace(/\&nbsp\;/gis, '');
        // const content_test_latex = await this.latexToImageSvgService.convertHtmlForWord(content_test);
