...
export class ImportGiangvienComponent implements OnInit {
        ...
        if (event.target.files[0]) {
            ...
            if (this.helperService.checkTypeFile(['application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'application/vnd.ms-excel'], file.type)) {
                readImportFile(file, this.notificationService, 'arrayBuffer', (localUrl) => {
                    ...
                    data.forEach(f => {
                        if (f[1] && f[2] && f[3] && f[4] && f[5] && f[7]) {
                            if (f[4]) {
                                if (!object_check_duplicate[f[4].trim()]) {
                                    object_check_duplicate[f[4].trim()] = true;
                                    filterData.push(f);
                                }
                            }
                        }
                ...
                this.notificationService.toastWarning("Sai định dạng file, yêu cầu file .xlsx. Vui lòng tải file mẫu danh sách import giảng viên");
            }
        }
    }
    ...
    convertData(data) {
        ...
        data.forEach((f, key) => {
            ...
            const index = this.list_donvi.findIndex(m => this.helperService.slugVietnamese(m.title) === this.helperService.slugVietnamese(f[5].trim()));
            if (index !== -1) {
                user_['name_khoa'] = this.list_donvi[index].title;
                user_['donvi_chuyenmon_id'] = this.list_donvi[index].id;
            }
    ...
    startImportSinhvien() {
        if (this.list_giangvien && this.list_giangvien.length) {
            const request: Observable<any>[][] = [];
            let i = 0;
            request[i] = [];
            this.list_giangvien.forEach(f => {
                const user = {
                ...
                const userProfile = {
                    ...
                    teacher: f.teacher,
                    donvi_chuyenmon_id: f.donvi_chuyenmon_id
                };
            ...
            if (request.length) {
                this.loopAddSinhvien(0, request).subscribe(() => {
                    ...
                    this.getStatusObject();
                    this.notificationService.toastSuccess("Đã hoàn thành quá trình import, vui lòng kiểm tra lại danh sách giảng viên đã import");
                })
            } else {
                this.notificationService.toastWarning("Không có giảng viên chưa import");
            }
        }
    }

    requestSinhvien(sinhvien: data_sinhvienImport, user, userProfile): Observable<any> {
        const request = this.userService.getUserByCol('email', user.email).pipe(
            mergeMap(_user => {
                if (_user.length) {
    ...
    getStatusObject() {
        if (this.list_giangvien) {
            this.status_object = {
                ...
                password_false: this.list_giangvien.filter(m => m['status_import'] === -3).length,
            }
        }
    }
