...
export class DongboDulieuComponent implements OnInit {
    ...
    downLoadExClass() {
        this.fileService.getFileContent('..\\assets\\files\\mau_file_import_LMS-LCMS\\Mau 05 - Import TKB GV toan khoa (thay mau 05-08 cu).xls').subscribe(res => {
            saveAs(res, 'Mau 05 - Import TKB GV toan khoa (thay mau 05-08 cu).xls');
        });
    }
                    ...
                    forkJoin([
                        ...
                        next: ([_resUser, _resKhoa]) => {
                            ...
                            if (_resKhoa[0]) {
                                ...
                                data_class.donvi_chuyenmon_id = _resKhoa[0].category_ids;
                ...
                if (!objectDuplicate[f[1].concat("_", f[2])]['duplicate']) {
                    if (!email[f[1].concat("_", f[2])]) {
                        ...
                        if (!objectStudentCode[f[1]]) {
                            ...
                            if (ar_student_f_request[j].length === 3) {
                                j = j + 1;
                                ar_student_f_request[j] = [];
                            }
                        } else {
    ...
    readyToDownLoadCourse() {
        ...
        forkJoin([
            ...
            next: ([_donvi, _nganh_bomon]) => {
                const data = [];
                this.loopGetCouse(data, 1, 1000, _donvi, _nganh_bomon);
            },
            error: () => {
                this.displayModal = false;
    ...
    loopGetCouse(data: ElnKhoaHoc[], page: number, count: number, donvi: DonVi[], _nganh_bomon: ElnChuyenMuc[]) {
        if (data.length < count) {
            ...
            this.elnKhoaHocService.getKhoaHocByPageNew(arrayCondition, page, setCondition).pipe(mergeMap(_resCourse => {
                ...
                next: _res => {
                    this.loopGetCouse(data.concat(_res.data), page + 1, _res.recordsFiltered, donvi, _nganh_bomon);
                },
                error: () => {
            ...
            this.donwloadCouseToExcel(data, donvi)
        }
    }

    donwloadCouseToExcel(data_course, list_donvi_chuyenmon) {
    ...
    loopGetTeacherForClass(_data_teacher, page, count, _data_sql, data_monhoc, data_class) {
        if (_data_teacher.length < count) {
            ...
            _data_sql.forEach((f, key) => {
                ...
                if (f[4].trim().indexOf('(') !== -1 && f[4].trim().indexOf(')') !== -1) {
                    f[4].trim().replace(/\(.*?\)/g, _res => {
                        kyhieu = _res.replace(/\(|\)/g, '');
                        return _res;
                    })
                } else {
                    if (f[4].trim().indexOf('(') !== -1) {
                        const tmp_kyhieu = f[4].trim().replace(/\(+/g, '#_#');
                        kyhieu = tmp_kyhieu.split('#_#')[1];
                    }
                }
