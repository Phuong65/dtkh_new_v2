...
export class SinhvienComponent implements OnInit {
        ...
        if (dmClassStudent && dmClassStudent.length) {
            ...
            this.fileService
                ...
                .subscribe((res) => {
                    saveAs(
                        res,
                        'Mau 06 - Import sinh vien vao 1 lop hoc phan.xls'
                    );
                });
        }
    }
    ...
    changeInputImport(event) {
        if (event.target.files[0]) {
            ...
            reader.onloadend = (event) => {
                ...
                this.progressValue = 0;
                this.waitting_title =
                    'Đang lấy dữ liệu sinh viên, vui lòng chờ';
                this.loopGetStudentClassToCheck(
                    data_student,
                    1,
                    1000,
                    data.filter((m) => m[1] && m[2] && m[3])
                );
            };
        }
    }

    loopGetStudentFromIctu(
        data_student,
        page: number,
        count: number,
        data_sql
        ...
        if (data_student.length < count) {
            ...
            this.convertIctuStudentData(data_sql, data_student);
        }
    }

    loopGetStudentClassToCheck(
        data_student,
        page: number,
        count: number,
        data_sql
        ...
        if (data_student.length < count) {
            ...
            this.convertSqlStudentData(data_sql, data_student);
        }
    }

    convertIctuStudentData(data_sql, data_student) {
        ...
        data_sql.forEach((f, key) => {
            const key_object = f[1].toString().trim().toLowerCase();
            if (!object_duplicate[key_object]) {
                object_duplicate[key_object] = false;
        ...
        this.loopGetStudentExistFromIctu(_data[0], _data, 0, [], list_data, data_student);
    }

    loopGetStudentExistFromIctu(data, datas, key: number, _student: ElngUserProfile[], convert_data, data_student) {
        if (key < datas.length) {
            ...
            this.elngUserProfileService.getUserProfileByPageNewV2(condition).pipe(
                mergeMap((_student_res) => {
                    const user_ids = [0];
                    ...
                    return this.userService.getUserByPageNew(condition_user).pipe(
                        mergeMap((_res_user) => {
                            ...
                            return of(_student_res);
                        })
                    );
                })
            ).subscribe({
                next: (_res) => {
                    this.loopGetStudentExistFromIctu(datas[key + 1], datas, key + 1, _student.concat(_res.data), convert_data, data_student);
                },
                error: () => {
                    this.displayModal = false;
                    this.noitifi.toastError('Lỗi kết nối, vui lòng thử lại');
            ...
            const _data_next = [];
            convert_data.forEach((f, key) => {
                const index = _student.findIndex((m) => m.student_code === f['student_code']);
                if (index !== -1) {
                    if (!_student[index].user_id || _student[index]['_no_user']) {
                    ...
                    const _student_sql = {
                        ...
                        user_info: {
                            ...
                            student_code: _student[index].student_code,
            ...
            this.list_student_import = _data_next;
            this.show_count_status.no_data = this.list_student_import.filter(
                (m) => m['status_import'] === 'no_data'
            ).length;
            this.show_count_status.not_import = this.list_student_import.filter(
                (m) => m['status_import'] === 'not_import'
            ).length;
            this.show_count_status.exist = this.list_student_import.filter(
                (m) => m['status_import'] === 'exist'
            ).length;
            this.show_count_status.sum_data = this.list_student_import.length;
            this.displayModal = false;
        }
    }

    convertSqlStudentData(data_sql, data_student) {
        data_sql.splice(0, 1);
        const object_duplicate = {};
        const list_data = [];
        let i = 0;
        ...
        this.waitting_title = 'Đang kiểm tra dữ liệu sinh viên, vui lòng chờ';
        this.loopGetStudentExist(_data[0], _data, 0, [], list_data, data_student);
    }

    loopGetStudentExist(data, datas, key: number, _student: ElngUserProfile[], convert_data, data_student) {
        if (key < datas.length) {
            this.progressValue = ((key + 1) / datas.length) * 100;
            const condition: ConditionOption = {
                condition: [],
            ...
            this.elngUserProfileService.getUserProfileByPageNewV2(condition).pipe(
                ...
                next: (_res) => {
                    this.loopGetStudentExist(datas[key + 1], datas, key + 1, _student.concat(_res.data), convert_data, data_student);
                },
                error: () => {
                    this.displayModal = false;
                    this.noitifi.toastError('Lỗi kết nối, vui lòng thử lại');
    ...
    onDownLoadFromIctuApi() {
        this.noitifi.isProcessing(true);
        this.classesService.getStudentClassesByIctu(Number(this.classSelected.sync_class_id)).subscribe({
            next: (_sync_student) => {
                ...
                this.loopGetStudentFromIctu(data_student, 1, 1000, data_sync_student);
            },
            error: () => {
                this.noitifi.isProcessing(false);
    ...
    startImportStudentToClass() {
        this.noitifi
            .confirm(
                'Bạn có chắc chắn muốn import không?',
                'Xác nhận hành động',
                [BUTTON_YES, BUTTON_NO]
            )
            .then((a) => {
                if (a.name === 'yes') {
                    ...
                    if (this.ghide_student) {
                        this.list_student_import.forEach((f) => {
                            if (f['status_import'] !== 'no_data') {
                                if (data[i].length < 5) {
                                    data[i].push(f);
                                } else {
                                    i = i + 1;
                        ...
                        this.list_student_import
                            .filter((m) => m['status_import'] !== 'exist')
                            .forEach((f) => {
                                if (f['status_import'] !== 'no_data') {
                                    if (data[i].length < 5) {
                                        data[i].push(f);
                    ...
                    this.loopAddStudentClass(data[0], data, 0);
                }
            });
    }

    loopAddStudentClass(data: ClassStudent[], datas: any[], key) {
        if (key < datas.length) {
            this.progressValue = ((key + 1) / datas.length) * 100;
            ...
            data.forEach((f) => {
                ...
                if (f['id']) {
                    request.push(
                        this.classStudentService
                            .updateClassStudent(f['id'], _student)
                            .pipe(
                                catchError(() => {
                                ...
                                mergeMap(() => {
                                    f['status_import'] = 'done';
                                    return of(null);
                                })
                            )
                    );
                } else {
                    const condition_check_student: ConditionOption = {
                        condition: [
                            {
                                conditionName: 'class_id',
                                condition: OvicQueryCondition.equal,
                            ...
                            },
                            {
                                ...
                                condition: OvicQueryCondition.equal,
                                value: _student.student_id.toString(),
                                orWhere: 'and',
                            },
                        ],
                        set: [],
                    ...
                    request.push(
                        this.classStudentService
                            .getClassStudentByPageNew(condition_check_student)
                            .pipe(
                                mergeMap((_student_check) => {
                                    if (_student_check.recordsFiltered === 0) {
                                        return this.classStudentService
                                            .addClassStudent(_student)
                                            .pipe(
                                                catchError(() => {
                                                    f['status_import'] =
                                                        'failed';
            ...
            if (request.length) {
                ...
                this.loopAddStudentClass(datas[key + 1], datas, key + 1);
            }
        } else {
            this.displayModal = false;
