...
@Component({
    selector: 'app-thongke-diemthuongxuyen-tueba-dttx',
    standalone: true,
    imports: [
        CommonModule, SharedModule, FormsModule, ReactiveFormsModule, TableModule, DialogModule, MatProgressBarModule, ButtonModule, RippleModule
    ],
    templateUrl: './thongke-diemthuongxuyen-tueba-dttx.component.html',
    styleUrls: ['./thongke-diemthuongxuyen-tueba-dttx.component.css']
})
export class ThongkeDiemthuongxuyenTuebaDttxComponent implements OnInit {
    ...
    getDataDongbo() {
        ...
        forkJoin([
            ...
            next: ([{ data }, dataPoint, _plan_activity, _plan]) => {
                this.typeView = 'data';
                console.log(_plan_activity);
    ...
    createAndAddRpt() {
        ...
        this.classStudentService.getClassStudentByCols(condition).pipe(switchMap(m => {
            ...
            return forkJoin<[ClassStudent[], ClassPlanActivitiesTests[], ClassPlanActivityStudentTests[], ClassPlanActivities[], ClassPlanActivityTuluan[], ClassPlanActivityStudentTests[], ClassStudentDiemdanh[], ClassPlans[]]>(
                [of(a[0]),
                of(a[1]),
            ...
            next: ([students, studentTxTn, studentCC, classPlanactivity, classPlanActivityTuLuan, studentCCNotDaugio, classStudentDiemdanh, _class_plan]) => {
                ...
                if (studentTxParam && studentCcParam) {
                    this.listDataClassStudent = students.map(m => {
                        ...
                        for (let i = 1; i <= weeks; i++) {
                            ...
                            const diem_bttn = mapData['bttn_max_tp']['week_' + i] === -1 ? 0 : mapData['bttn_max_tp']['week_' + i];
                            console.log(diem_bttn);
                        ...
                        mapData['bttn'] = this.replaceNumberThapPhan((bttn_tp_total / weeks));
                    ...
                    console.log(this.listDataClassStudent);
    ...
    async btnExportExcel() {
        ...
        const config__diem = await firstValueFrom(this.courseConfigService.getCourseConfigByPageNew(condition_course_config));

        const default_config_tyle_diem = {
            cc: 15,
            bttn: 15,
            kynang: 20,
            thi: 50
        ...
        if (config__diem.recordsFiltered) {
            config__diem.data.forEach(f => {
                const index = Object.keys(default_config_tyle_diem).findIndex(m => m === f.key);
                if (index !== -1) {
                    default_config_tyle_diem[f.key] = f.value;
                }
            })
        }
        ...
        const datatx = this.listData.map((m, index) => {
            ...
            m.tyle = default_config_tyle_diem;
            ...
            const totalPoint = this.plussToDecimal(this.numToDecimal(m.cc > 0 ? m.cc : 0, 0.15), this.numToDecimal(m.bttn, 0.15), this.numToDecimal(totalConvert, 0.2))
        ...
        const datatxtp = this.listData.map((m, index) => {
            ...
            for (let i = 1; i <= weekUse; i++) {
                itemAdd['week' + i] = max_tp['week_' + i] && max_tp['week_' + i] > -1 ? max_tp['week_' + i] : 0;
            }
        ...
        if (datatx && datatxtp && columnDataTx) {
            await this.exportExcel.exportExcel(datatx, datatxtp, this._clases, this._clases.name, weekUse, columnDataTx, columnDataTxTp, this.tongKtKyNang, default_config_tyle_diem);
        }
    }
