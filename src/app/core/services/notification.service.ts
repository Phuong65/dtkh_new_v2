...
import { OvicMediaPlayerComponent } from '@modules/shared/components/ovic-media-player/ovic-media-player.component';
...
export class NotificationService {
	...
	public TURNMENU = new BehaviorSubject<any>({});

	private onCloseLeftMenu$ = new BehaviorSubject<boolean>(null);
	...
	tplOvicMediaPreview(data: OvicMedia): Promise<any> {
	...
	closeLeftMenu() {
		this.TURNMENU.next({ menu: false });
	}


	getCloseLeftMenu(): Observable<boolean> {
		return this.onCloseLeftMenu$.asObservable();
	}


	setCloseLeftMenu(close: boolean = false) {
		this.onCloseLeftMenu$.next(close);
	}
}
