...
export function shuffleArray<T> ( array : T[] ) : T[] {
	...
	const randomValues : Uint32Array = new Uint32Array( result.length );
...
type CloneObjectFunc = <T>( obj : T ) => T;
/**
 * Clone an object safety
 * @param object Object Or array.
 * */
const cloneObject : CloneObjectFunc = <T> ( object : T ) : T => {
	return typeof Is.object( object ) || Is.array( object ) ? JSON.parse( JSON.stringify( object ) ) : object;
}
...
export class HelperService {
	...
	stringToDateSql ( date : string , nosec : boolean = false ) : string {
		const d = new Date( date );
		return this.formatSQLDateTime( d , nosec );
	}

	formatSQLDateTime ( date : Date , nosec : boolean = false ) : string {
		...
		return nosec ? `${ y }-${ m }-${ d } ${ h }:${ min }` : `${ y }-${ m }-${ d } ${ h }:${ min }:${ sec }`;
	}
	...
	checkTypeFile ( fileTypes : string[] , types : string ) {
		if ( fileTypes.length ) {
			const index = fileTypes.findIndex( m => m === types );
			if ( index !== -1 ) {
				return true;
			}
			return false;
	...
	replaceImageSrc ( stringHtml : string , img : any ) {
		if ( stringHtml && Array.isArray( img ) ) {
			const regexSrcG = /src="(.*?)"/g;
			const noSrc     = stringHtml.toString().replace( regexSrcG , '' );
			const newHtml   = noSrc.replace( /data-id="(.*?)"/g , ( res ) => {
				const dataId      = res.replace( /d|a|t|-|i|=|"/g , '' );
				const index_image = img.findIndex( m => m.id.toString() === dataId );
				if ( index_image !== -1 ) {
					return 'src="'.concat( img[ index_image ].idUpLoad.toString() , '"' );
				}
				return res;
			} );
			return newHtml;
	...
	replaceAwsImageHasIdAlready ( stringHtml ) {
		if ( stringHtml ) {
			const regexSrcG = /src="(.*?)"/g;
			const noSrc     = stringHtml.toString().replace( regexSrcG , _res => {
				const src_id = _res.replace( getLinkDownload_aws( '' ) , '' ).replace( "?token=".concat( this.auth.accessToken ) , '' ).replace( /\D/gi , '' );
				return 'src="'.concat( src_id , '"' );
			} );
			return noSrc;
		}
		return null;
	...
	moveLinkToCourse ( course_id : string ) {
		this.fileService.getFileLocalAsBlob( "..\\assets\\json\\front_end.json" ).subscribe( {
			next  : ( _res ) => {
				...
				reader.onloadend = ( event ) => {
					...
					const url      = json[ 'url' ].concat( json[ 'route_course' ] , course_id , '?token=' , this.auth.accessToken );
	...
	convertDateGbToDateSql ( date : string ) : string {
		let result = null;
		try {
